...
void Block::Draw(const WorldTransform& worldTransform, const ViewProjection& viewProjection) {
	...
	//transformationMatrixData_->World = MakeAffineMatrix(worldTransform.scale_, worldTransform.rotation_, worldTransform.translation_);
	//transformationMatrixData_->WVP = Multiply(worldTransform.matWorld_, *Camera::GetInstance()->GetTransformationMatrixData());
	...
	// DescriptorTableの設定
	DirectXCommon::GetInstance()->GetCommandList()->SetGraphicsRootDescriptorTable(2, TextureManager::GetInstance()->GetTextureSrvHandleGPU()[TESTBLOCK]);
...
void Block::ApplyGlobalVariables() {
...
void Block::ImGuiAdjustParameter() {
